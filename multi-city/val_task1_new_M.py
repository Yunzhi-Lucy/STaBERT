...
def task1(args):
    result_path = 'result/taskM'
    os.makedirs(result_path, exist_ok=True)

    task2_dataset_val = HuMobDatasetTask1Val()
...
#    device = torch.device("cpu")
    ...
    with torch.no_grad():
        for data in tqdm(task2_dataloader_val):
            ...
            output = model(data['d'], data['t'], data['input_x'], data['input_y'], data['time_delta'], data['len'], data['poi_categories'], data['poi_counts'], data['city'])
