...
def create_poi_mask(poi_categories, pad_value=0):
    """
    - input: poi_categories [batch, seq_len, num_pois]
    - output: poi_mask [batch, seq_len, num_pois]，1 means have this poi POI，0 means just filling
    """
...
class EmbeddingLayer(nn.Module):
    ...
    def forward(self, day, time, location_x, location_y, timedelta, poi_categories, poi_counts):
        ...
        embed = day_embed + time_embed + location_x_embed + location_y_embed + timedelta_embed + poi_final
        return embed
...
class LPBERT(nn.Module):
    ...
    def forward(self, day, time, location_x, location_y, timedelta, len, poi_categories, poi_counts):
        embed = self.embedding_layer(day, time, location_x, location_y, timedelta, poi_categories, poi_counts)
