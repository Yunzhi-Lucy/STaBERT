...
def collate_fn(batch):
    ...
    poi_categories = [item['poi_categories'] for item in batch]  
    poi_counts = [item['poi_counts'] for item in batch]  
    ...
    poi_categories_padded = pad_sequence(poi_categories, batch_first=True, padding_value=0) 
    poi_counts_padded = pad_sequence(poi_counts, batch_first=True, padding_value=0)
    ...
    return {
        ...
        'poi_categories': poi_categories_padded,
...
def task1(args):
#    name = f'batchsize{args.batch_size}_epochs{args.epochs}_embedsize{args.embed_size}_layersnum{args.layers_num}_headsnum{args.heads_num}_lr{args.lr}_seed{args.seed}'
    ...
    task1_dataset_train = HuMobDatasetTask1Train(path=args.dataset, poi_path=args.poi_dataset)
    task1_dataloader_train = DataLoader(task1_dataset_train, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn, num_workers=0, pin_memory=False)
...
#    device = torch.device("cpu")
    ...
    for epoch_id in range(args.epochs):
        for batch_id, batch in enumerate(tqdm(task1_dataloader_train)):
            ...
            batch['poi_categories'] = batch['poi_categories'].to(device)
            batch['poi_counts'] = batch['poi_counts'].to(device)

            output = model(batch['d'], batch['t'], batch['input_x'], batch['input_y'], batch['time_delta'], batch['len'], batch['poi_categories'], batch['poi_counts'])
